  ...
  <div class="food" transition="move" v-show="isShow" v-el:food>
    <div class="food-content">
      ...
      <div class="content">
        ...
        <!-- 注意点击加入购物车后再次点击减但仍然是加的问题 ，要去掉动画效果transition：“fade”-->
      ...
      <div class="rating">
        ...
        <ratingselect :desc="desc"
                      ...
                      @switch-only-content="switchOnlyContent"
...
<script>
  ...
  export default{
    ...
    methods: {
      showOrHide (isShow) { // 显示或者隐藏当前food详情
        this.isShow = isShow
...
//        刷新列表
        this.$nextTick(() => { // 刷新列表，否则查看了评论后会有大段空间
          this.scroll.refresh()
        })
      },
//      更新selectType
...
//        刷新列表
    ...
    computed: {
      filterRatings () {
//        如果没有数据结束 ，解决报错filter  undefined的问题
        ...
        return ratings.filter(rating => {
          ...
          if (selectType===2) {  //  如果点击的是全部
            /* if (!onlyContent) {
              return true
            } else {
              return text.length>0
            } */
            return !onlyContent || text.length>0 // 看完是否是查看有内容的评价再看其数量
          } else { // 如果点击的不是全部
