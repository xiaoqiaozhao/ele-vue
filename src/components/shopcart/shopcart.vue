<template>
  <div class="shopcart">
    ...
    <!-- v-show="listShow"表示根据计算选中菜单项的数量来控制菜单列表的显示隐藏，数量为0就是隐藏 -->
...
//      下部的显示内容，是否够起送
      payText () {
        console.log(this, this.minPrice)
...
//      下部的点菜列表
      listShow () {
        ...
        if (this.isShow) {
          this.$nextTick(() => { // 延迟执行
            if (!this.scroll) { // 当没有滚动条时才会创建滚动条
              this.scroll = new BScroll(this.$els.listContent, {
                click: true // 必须写，不写的话不会响应点击事件
              })
            } else {
              this.scroll.refresh() // 刷新形成滚动条
            }
          })
        }
        return this.isShow
    ...
    methods: {
//      下部的点菜列表切换
      toggleList () { // foodList在goods组件中定义的
...
//          瞬间移动动画至点击位置
//          分析：小球的移动是由inner元素（决定水平方向）和外层包裹元素ball（el决定垂直方向）共同控制的
          el.style.transform = `translate3d(0,${offsetY}px,0)`
          el.style.webkitTransform = `translate3d(0,${offsetY}px,0)`  // 为了兼容

          const innerEl = el.children[0]
          innerEl.style.transform = `translate3d(${offsetX}px,0,0)`
          innerEl.style.webkitTransform = `translate3d(${offsetX}px,0,0)`  // 为了兼容
...
//        动画开始时:指定动画结束的位置
        enter (el) {
//          需要异步处理,不然没有过渡直接就到结束位置了
          this.$nextTick(() => {
            el.style.transform = `translate3d(0,0,0)`
            el.style.webkitTransform = `translate3d(0,0,0)`  // 为了兼容
            const innerEl = el.children[0]
            innerEl.style.transform = `translate3d(0,0,0)`
            innerEl.style.webkitTransform = `translate3d(0,0,0)`  // 为了兼容
          })
        },
//        动画完成之后,做些收尾的工作:隐藏小球
        afterEnter (el) {
...
//          隐藏小球
          ball.isShow = false
          ball.startEl = null
//          这里会产生一个问题:虽然ball.isShow变为false了,但是页面上还是有小球,没有隐藏:
//          这是因为:在动画生命周期回调函数中更新状态,页面不改变
//          解决方法:通过原生dom让el隐藏
